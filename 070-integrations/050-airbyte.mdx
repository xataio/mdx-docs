...
Next you need to create the table in Xata with the corresponding schema. Please note that databases, tables and schemas in Xata are not created by the connector, you need to take care of that before ingesting data. One important convention is used, as highlighted [here](#3-configure-source), that the name of a table and the dataset name in Airbyte must be same.
...
For simplicity, the schema is publicly available and you can use our [CLI](/getting-started/cli) to create the table `Companies` with the schema.
...
The illustration and the next steps assume you don't have the source created yet. If you already have a CSV source created, please skip to [step 4](#4-select-destination).
...
![Setup Xata as Destination](/docs/images/docs/integrations/airbyte/setup-xata-destination.png)
