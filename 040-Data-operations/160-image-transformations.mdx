...
Xata supports transformations for images stored in Xata's `File` and `File[]` [column types](/docs/sdk/file-attachments).

Transformations are specified by modifying the `File`'s URL by adding `/transform/height=100,width=200/{fileId}`

## Example

(Vanity idea: gif which animates pulling the url apart and inserting the transform. And also animates the actual image :fire:. Or it could be react code not a gif. Might be too distracting)
...
transformImage('https://us-east-1.storage.xata.sh/4u1fh2o6p10blbutjnphcste94', {
  height: 100,
  rotate: 180,
  format: 'webp'
});
...
record.movieThumbnail.transform({
  height: 100,
  rotate: 180,
  format: 'webp'
});
// TODO: Richard: WRONG this an object
// => https://us-east-1.storage.xata.sh/transform/rotate=180,height=100,format=webp/4u1fh2o6p10blbutjnphcste94
```

```python
# TODO: Richard: missing import?
xata.files().transform("https://us-east-1.storage.xata.sh/4u1fh2o6p10blbutjnphcste94", {
  "height": 100,
  "rotate": 180,
  "format": "webp"
})
...
const { metadataUrl } = record.movieThumbnail.transform({
  height: 100
});
// => https://us-east-1.storage.xata.sh/transform/format=json,height=100/4u1fh2o6p10blbutjnphcste94
...
```python
# TODO: Richard: missing import?
metadata_url = xata.files().transform("https://us-east-1.storage.xata.sh/4u1fh2o6p10blbutjnphcste94", {
  "format": "json",
  "height": 100,
})
# => https://us-east-1.storage.xata.sh/transform/rotate=180,height=100,format=webp/4u1fh2o6p10blbutjnphcste94

# todo: Richard: fetch it
```
...
This refers to the Device Pixel Ratio (DPR). The default is 1. This is a multiplier (similar to a flat scale) for width/height that makes it easier to specify higher-DPI sizes in `img srcset`. `dpr` needs to be used in tandem with a (@snide, what is a dimension transform?) dimension transform to take effect.
...
/>
@snide: I'm not sure about these double examples. Options I see: Kill the second example, or make it more obvious why there are two examples. Maybe by pulling out the "Note that the rendered example retains the original aspect-ratio, but doesn't grow beyond the original size" into markdown?
<TabbedCode tabs={['TypeScript', 'Python', 'URL']}>
