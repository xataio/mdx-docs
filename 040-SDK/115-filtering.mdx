...
This example will demonstrate how to use multiple operators.
We want to get all records that:

- IP range starts with `202.8.*` or `29.171.152.*`
- attempted to login on URL path `usr/login`
...
const records = await xata.db.access_logs
  .filter({
    $all: [
      ...
      // attempted to login with PUT or POST
      { method: { $any: ['PUT', 'POST'] } },
      // login url_path exact match
