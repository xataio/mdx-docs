...
Following our previous examples with a `Posts` table, let's assume that each post has a `social` field of type `JSON` which contains the times the post was shared on different social networks:
...
Filtering posts by the sharing metrics on LinkedIn is as simple as:
