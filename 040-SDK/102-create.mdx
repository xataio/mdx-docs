...
Note, the create method fails if the record ID already exists.

The SDK returns the created record. The response looks like this:
...
It is possible to provide IDs for each of the records. If records with the same IDs already exist, the bulk operation is canceled and an error is returned. The bulk create method fails if the record ID already exists.
