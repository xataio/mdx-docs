...
In the previous example it was already hinted, that `"all_sales": {"count": "*"}` will return the amount of records of the selected table. 
For simplicity, the following example will solely show how the retrieve the total number of records stored in the table `sales`:
...
console.log(res.summaries[0].total)
```
