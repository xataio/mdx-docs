...
In the context of the given table, a column represents an attribute that you want to perform a calculation on. Assigning `product` as a column yields one result row for each unique value within the `product` column. This implies you retrieve two rows: one representing `basketball` and the other, `shirt`.
