...
The endpoint has two concepts that are important to understand: _columns_ and _summaries_. If you're already familiar with aggregations and `GROUP BY` in SQL, you can skip this section.
...
In the context of the given table, a column represents an attribute for which you desire to perform a calculation. Assigning `product` as a column would yield one result row for each unique value within the `product` column. With the table above, this implies you would retrieve two rows: one representing `basketball` and the other, `shirt`.
