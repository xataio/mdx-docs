...
Depending on the file type, some browsers preview the file instead of downloading it. Setting the `download` property will make the browser to always download the file. The value allows you to assign a name, and optionally an extension to the file as it downloads.

<TabbedCode tabs={['TypeScript', 'Python', 'URL']}>

```ts
const transformedImageUrl = transformImage(
  'https://us-east-1.storage.xata.sh/4u1fh2o6p10blbutjnphcste94',
  {
...
Example: Download [xata-cat.jpg](https://us-east-1.storage.xata.sh/transform/download=xata-cat.jpg,format=jpeg/4u1fh2o6p10blbutjnphcste94)
