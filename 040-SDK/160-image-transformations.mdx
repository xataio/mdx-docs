...
Once a transformation occurs, it is common to need the dimensions of the new image. `metaDataUrl` and `signedMetaUrl` (if used signed URLs) are available when making transformations from a file on a record.
