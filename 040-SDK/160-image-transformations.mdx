...
Once a transformation occurs, it is common to need the dimensions of the new image. `metadataUrl` and `signedMetadataUrl` (if using signed URLs) are available when making transformations from a file on a record.
...
const metadata = fetchMetadata(metaDataUrl);

console.log(metadata.height); // 100px

//
// Generic TypeScript function useful for fetching metadata
//
