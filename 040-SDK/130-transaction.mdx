...
We support the `ifVersion` flag to avoid overwriting unexpected versions of the record.
...
const result = await xata.transactions.run([
  {
    insert: {
      table: 'titles',
      where: { id: 'rec_cfl4g6v838og05h0iv6g', version: ifVersion },
      record: { originalTitle: 'An updated title' }
    }
  }
]);
```
  ...
  "operations": [
    {
      "insert": {
        "table": "titles",
        "where": { "id": "rec_cfl4g6v838og05h0iv6g", "version": if_version },
        "record": { "originalTitle": "An updated title" }
      }
    }
  ]
...
Delete is used to remove records. Delete can operate on records from the same transaction, and will not cancel a transaction
...
Get is used to retrieve a record by ID. A get operation can retrieve records created in the same transaction but will not cancel
...
- To avoid the SQL proxy timeout, delete in small batches. It is recommended to delete no more than 10,000 records per batch.
