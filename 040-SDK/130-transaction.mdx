...
The single-call transaction endpoint allows you to execute multiple operations together as a single unit. In this guide, we'll run through why transactions are important and run through Xata's single-call transaction endpoint.
...
The number of operations allowed in a single-call transaction is [limited](/docs/sdk/transaction#limitations). It is _not_ possible to chain multiple single-call transactions to logically form a larger one. Each single-call transaction request runs atomically and either succeeds or fails in its entirety.
