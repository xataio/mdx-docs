...
### Step 1: Create a record with an empty file (server side)

On the server side use `xata.db.myTableName.create` to create a record with an empty file and simultaneously request an `uploadUrl`.
...
### Step 2: Upload the file using a `PUT` request to the `uploadUrl` (client side)
...
try {
  ...
  try {
    ...
    await fetch(record.myUploadUrl, { method: 'PUT', body: file });
  } catch (error) {
