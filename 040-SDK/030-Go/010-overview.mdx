...
<Alert status="warning">
  The Go SDK is currently in *alpha*. It does not include all endpoints and is subject to potential breaking changes
  before reaching its `1.0.0` stable General Availability (GA) release.
</Alert>
...
You can run the snippet above by pulling the example code from the `xataio/xata-go` repository
...
- Currently, there is no option to change the [consistency](/docs/concepts/data-model#consistency). The default setting is `strong`.

## Contribute

Our SDK documentation is open source, and we encourage contributions from the community. To learn more about how you can contribute, visit our [Get involved documentation](/docs/resources/contribute#contribute-code) or explore the [GitHub](https://github.com/xataio/xata-go) repository.
