...
- `getMany()`: returns the requested number of records for the query in a single response. The default is 20 records, but you can change it by modifying the pagination size. The pagination limit is 50,000 records.
...
The offset-page pagination limit is 50,000 records (about 250 pages). In most cases, you should use the cursor-based pagination.
