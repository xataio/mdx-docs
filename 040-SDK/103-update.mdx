...
Great! We can now update data safely in our databases. Let's now explore how we [delete data](/docs/sdk/delete) from a database. Alternatively, we can also look into [inserting data](/docs/typescript-client/insert) or using [transactions](/docs/typescript-client/transaction) to perform multiple operations with a single call. We've got guides for each of these operations.
