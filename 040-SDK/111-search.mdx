...
Regardless of client, the above search will return results in the below shape. The returned result adds an additional `xata` field which contains metadata about the search that includes:

- the table in which the result was found.
...
Note the `xata.highlight.address` field with marks our search term (`New Set`) with surrounding `<em>` tags.
...
With the `fuzziness` set to `0` the misspelling will no longer match for `Keanu`.

You can also increase the `fuzziness` to accept a larger amount of typos.
...
The search API provides a configurable page size and offset-based pagination. To skip pages set the offset to a multiple of the page size.
