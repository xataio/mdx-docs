...
SQL access in Xata is achieved via HTTP/S Proxy. Tools that use wire protocol with a connection string to PostgreSQL are not compatible with Xata's SQL Proxy.
You can use SQL in Xata with any of the following methods:

- From the built-in PostgreSQL Playground in the Xata Web UI.
- With API calls to your branch's `/sql` endpoint using Web Clients such as Postman.
- Using the *sql* methods provided in the Typescript and Python SDKs.
- Through [Kysely](/docs/integrations/kysely) and [Drizzle](/docs/integrations/drizzle) integrations.