...
const { databaseName } = await api.database.createDatabase({
  workspace,
  database: 'database',
  data: { region: 'eu-west-1' }
});

await api.branches.createBranch({
  workspace,
  region: 'eu-west-1',
...
const recordId = await api.records.insertRecord({
  workspace,
  region: 'eu-west-1',
  database: databaseName,
  branch: 'branch',
...
const record = await api.records.getRecord({
  workspace,
  region: 'eu-west-1',
  database: databaseName,
  branch: 'branch',
