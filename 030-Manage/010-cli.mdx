...
```bash
curl xata.io/lite/install.sh | sh
```
...
<Alert status="warning">
  The `xata schema` command is not supported for use with non-postgres-enabled databases which are deprecated. The command will continue to work for existing non-postgres-enabled databases.
  For Postgres-enabled databases, we recommend using `pg_dump --schema-only` and `pg_restore` to migrate your schema.
</Alert>
...
[2]: /docs/getting-started/cli#schema
[3]: https://app.xata.io
[4]: https://nodejs.org/
