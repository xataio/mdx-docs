...
The following examples assume the `client` variable is an instance of `XataClient` with the correct credentials. Please refer to the [Configuration](/docs/developer-guide/python-sdk/overview#configuration) page to learn more about initializing the SDK.
...
- `xata.records().update_with_id()_`: table name (str), record id (str), record (dictionary)
- `xata.records().upsert_with_id()`: table name (str), record id (str), record (dictionary)
...
resp = xata.records().update_with_id("Avengers", "spidey-1", record)
assert resp.is_success()
...
resp = xata.records().upsert_with_id("Avengers", "hulk-1", record)
...
resp = xata.records().upsert_with_id("Avengers", "spidey-1", record)
...
The Bulk Processor is a helpful tool for performing bulk operations using the Xata Python client. It abstracts away the complexity of managing bulk operations, making it easier to perform large-scale insert operations. Using the [Bulk Processor](/docs/developer-guide/python-sdk/bulk-processor) is recommended for bulk operations in Xata.
...
The `ask` endpoint uses search or similarity search algorithms to find relevant information from your database. Please refer to [this page](/docs/developer-guide/typescript-client/ask) to get more information about the endpoint.
