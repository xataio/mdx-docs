...
Multiple record update operations can be performed in a single call using the [Transaction](/docs/developer-guide/typescript-client/transaction) API.

## Next Steps

Great! We can now update data safely in our databases. Let's now explore how we [delete data](/docs/developer-guide/typescript-client/delete) from a database. Alternatively, we can also look into [inserting data](/docs/developer-guide/typescript-client/insert) or using [transactions](/docs/developer-guide/typescript-client/transaction) to perform multiple operations with a single call. We've got guides for each of these operations.
