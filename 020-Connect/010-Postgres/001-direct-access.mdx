...
> This is a beta feature that requires a workspace administrator to turn on the `Direct access to Postgres` feature flag within their **workspace settings page**. Once enabled, any _new_ databases created within that workspace will have the option to enable the feature. Existing databases that weren't set up with direct access can not be migrated to allow these features without direct [Xata support](/docs/resources/support) involvement.
...
Xata offers Postgres in a variety of [pricing formats](/pricing). Shared cluster plans limit statements to guaruntee isolation in a multi-tenant environment. [Dedicated cluster](/docs/dedicated-cluster) plans provide more flexibility.
...
Xata reserves the `xata_` column prefix for [special columns](/docs/concepts/data-model#special-columns) and rejects any columns with the prefix.
