...
Xata allows you to your database branch via the standard Postgres wire protocol. You can use this connection method to use almost any tool that supports Postgres, including `psql`, `pg_dump / pg_restore`, client drivers in various programming languages, ORMs, BI tools, and more.

The SQL commands that you send over the wire protocol behave identical to the way they behave on any other Postgres installation or service. However, some functionality is restricted, especially on shared clusters. This page documents the limitations you can expect.

This page shows you how to connect to Postgres directly, by explaining how a connection string is built, the different Xata parameters map to Postgres URLs and examples to connect via [psql](https://www.postgresql.org/docs/current/app-psql.html).
...
- Schemas are not supported. Data content and objects are restored under the virtual public schema of the target branch.

## Supported statements

Xata offers different approaches for connecting to Postgres, from SQL over HTTP to pure Postgres over the wire protocol to a shared or [dedicated cluster](/docs/dedicated-cluster).
...
Depending on your Workspace's [plan](/docs/concepts/pricing#pricing-plans) and cluster, you have different connection limits for the wire protocol:
...
Xata reserves the `xata_` column prefix for [special columns](/docs/concepts/data-model#special-columns) and the UI rejects any custom created columns with the prefix. Avoid naming columns with this prefix as there will be functional implications.
