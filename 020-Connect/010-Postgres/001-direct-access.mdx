...
> This is a beta feature that requires a workspace administrator to turn on the `Direct access to Postgres` feature flag within their **workspace settings page**. Once enabled, any _new_ databases created within that workspace will have the option to enable the feature. For now, existing databases that weren't set up with direct access initially will need to be ported into a brand new database if you wish to utilize these features.
...
The SQL commands that you send over the wire protocol behave identical to the way they behave on any other Postgres installation or service. We maintain a list of [supported statements](#supported-statements) which limits some statements within our default shared clusters. Full access is available with our Dedicated Cluster plans.
...
### Type text constraint violation

If you created a column of type `text` in your table, through the UI, a constraint limiting to a 1000 characters is created with it.
The limitation affects search, Xata does not replicate columns that contain more than 1000 characters, hence these values will not arrive in Elasticsearch.

You will notice the constraint violation if, e.g. you created a column `content` in a table `Urls`, with following error message:

```text
new row for relation "urls" violates check constraint "urls_xata_text_length_content"
```

To remove this constraint, you need to drop it through the wire protocol:
