...
Xata reserves the `xata_` column prefix for special columns such as record id and timestamp metadata.
While it is possible to create columns named after this prefix and import data into them from CSV, it is strongly advised to avoid this column naming pattern.
Introducing columns of this naming pattern can result in incompatibilities with other features and components in the platform.
