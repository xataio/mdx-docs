...
In the Zap editor, click the **Action** step, or click the plus **+** icon to add an action to your Zap. Search for the Xata Integration and click the **Event** dropdown menu. The Event selection list presents the following options:
...
- Starting with version 1.0.1, the integration supports columns of the `json` type. To store JSON documents correctly, use the standard double quotation mark `"` for string values. Be cautious with Trigger data sources that produce JSON with non-standard left and right double quotes, as these are not compliant with the [RFC 7159](https://www.rfc-editor.org/rfc/rfc7159.html) standard.
