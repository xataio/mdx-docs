...
- Since v1.0.1 the integration supports the `json` column type. Note that in order to store json documents, the character `"` should be used as the quotation mark. Some Trigger data sources may generate JSON using left and right double quotation marks, which are not [RFC 7159](https://www.rfc-editor.org/rfc/rfc7159.html)-compliant documents.
