...
- You will need to [install the Xata CLI and authenticate](https://xata.io/docs/getting-started/installation)
...
1.  Create a new Cloudflare Workers endpoint:

    ```sh
    npm create cloudflare@latest
    ```

    Accept the default prompt options.

2.  After the command has completed, navigate to the `getting-started-cloudflare-workers` directory, and run the application:
...
## Copy .env file vars to .dev.vars

After running `xata init`, you should have a `.env` file at your app root that looks something like this:
...
## Query data from the worker

You'll need to initialize your Xata Client in your worker. There are two ways of initiailizing Xata client in Cloudflare workers:

1.  ES Modules (Recommended)
2.  Service Workers (Deprecated)
...
## Setting env vars in remote deployment

There are two ways to make the environment variables you've defined locally available in your remote deployment:
...
### Manually through the Cloudflare dashboard
