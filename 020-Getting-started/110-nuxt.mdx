...
Let's begin by creating a new Nuxt application, accepting the default prompt options:
...
Now the Nuxt app is ready to for Xata to be integrated into the codebase.
...
the posts stored in the database. This is achieved via the auto-generated `Posts` property which exposes a number of
helper functions. In this case, use the [`getAll`](https://xata.io/docs/typescript-client/get#the-typescript-sdk-functions-for-querying)
...
The Xata client instance is access using the `getXataClient` auto-generated function as it is in `server/api/posts.ts`.
...
information using `useRoute`. Then, make a request to the `/api/posts/[slug]` endpoint to fetch the single blog post information, passing the value
...
Let's add this functionality to `/api/posts` API server route:
...
get the value of the `q` querystring parameter, assigning it to a varable named `search`.
...
`useRoute` is used in a similar way to the single blog post view except this time the value of the `q` querystring is
retrieved from `route.query.q` and assigned to a variable named `search`. That is then effectively proxied onto the
`/api/post` API server route in the `useFetch` call where a second argument is added passing a `query` with a `q`
querystring set to the value of `search. This performs the search against the Xata database and returns the results in
...
search value has been already implemented, everything is in place.
