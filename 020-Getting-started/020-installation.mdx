...
When creating templates or examples for public use and cloning, avoid committing files that contain specific database API endpoint and client files. Configuration files such as `src/xata.ts`,`.xatarc` and the `.xata` migrations folder are tied to a specific database.
