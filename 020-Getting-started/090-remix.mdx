...
The completed [Remix and Xata code](https://github.com/xataio/examples/tree/main/apps/remix-getting-started) for this
...
Let's begin by creating a new Remix application, accepting the default prompt options:

```sh
npx create-remix@latest xata-remix
```

<Expand title="npx create-remix output">
...
With the Xata CLI installed and logged in and a new Remix application in place, use the Xata CLI to create a new
...
Once all the steps from the guide have been followed, updated `app/tailwind.css` with some additional rules:
...
Make sure the `<Outlet />` component is before the closing `</main>` element.

## Query and list the posts

Now we're ready to integrate Xata into the Remix codebase. Let's start by stripping back the landing page,
...
Next, let's updated the page to get all the posts using Xata, and
...
First, the existing import statement is updated to also bring in `json`, and a new import from
...
the post, and `description` for the textual descript of the post. Since `pubData` has been serialized to a string, convert
...
The `params` property on the `LoaderArgs` gives us the slug value from the URL via `params.slug`.
...
auto-generated `Posts` property to perform a query on the Posts table to find the
...
second options parameter with `fuzziness` set to `2`, which informs the search behavior to allow for two
...
Update the return value of `loader` to return both the `posts` and the value of `search`.
...
Begin by retrieving both the `posts` and the `search` values using the Remix `userLoaderData` hook.
