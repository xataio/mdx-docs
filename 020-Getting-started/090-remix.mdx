...
In this guide, we'll create a new [Remix][2] application and walk through adding Xata database and search
...
Next, a Remix framework [`loader`](https://remix.run/docs/en/main/route/loader) server-only function that provides
data to the route when renderingis defined and exported. Within that function an instance of the `XataClient`
...
which exposes a number of helper functions. In this case, we use the
...
Then, update the UI to display the results. If no records are present, we show a message
...
To handle the single posts identified by a `slug` we need to make use of Remix
