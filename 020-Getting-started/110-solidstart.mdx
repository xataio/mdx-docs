...
```tsx title="app/routes/_index.tsx" {1-3,5-17,20,19-59}
...
Import [`Match` and `Switch`](https://www.solidjs.com/docs/latest/api#switchmatch), and [`For`](https://www.solidjs.com/docs/latest/api#for) from Solid to be used when rending the UI. Then import
[`useRouteData`](https://start.solidjs.com/api/useRouteData) and the server-only [`createServerData$`](https://start.solidjs.com/api/createServerData) function to be used when retrieving data from Xata.
