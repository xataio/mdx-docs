...
The `Title` component is imported from `solid-start` to allow the page title to bve set. The `routeData` and
...
```tsx title="src/routes/posts/[slug].tsx" {1,15-24,32,36-45}
...
to be the value of `search` so if there is an active search, the user has some feedback.

The default submit behavior of a form is to perform a `GET` request to the current URL with any form inputs added to the query
...
The result of the changes mean your landing page UI looks like this:
