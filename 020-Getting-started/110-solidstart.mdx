...
Let's begin by creating a new SolidStart application, accepting the default prompt options:
...
Once the command has completed, go to the `xata-solidstart` directory and run the application:
...
from you SolidStart application. To do this, run:
...
Finally, replace the contents of `src/root.css` with the following:
...
Now you are ready to integrate Xata into the SolidStart codebase. Let's start by stripping back the landing page,
...
Next, let's update the page to get all the posts using Xata, and
list them within the page:

```tsx title="app/routes/_index.tsx" {1-3,5-17,19-54}
...
[`For`](https://www.solidjs.com/docs/latest/api#for) from Solid to be used when rending the UI. Then import
...
Next, define and export a `routeData` function. This returns a call to `createServerData$` which is a SolidStart function
that exists specifically for accessing resources only available on the server like databases. This function provides data to the route when rendering.
...
use the `xata` client instance to get all the posts stored in the database. This is achieved via the auto-generated `Posts` property
...
all the Post records. Finally, return the `posts` as the functions return value.
...
Finally, within the `Home` function, use the `useRouteData` hook to retrieve the `posts` data.
...
function. You also added the single post page making use of SolidStart dynamic segments where a `slug` was passed and used
