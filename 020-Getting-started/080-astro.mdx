...
A single post is accessed via the `slug` value of the post in the application's URL. For example, `/posts/awesome-blog-post` where `awesome-blog-post` is the slug. To do this, we need to use Astro's
...
```tsx astro="src/pages/posts/[slug].astro" {2,4-18,20-40}
---
import MainLayout from '../../layouts/main.astro'
```
...
The `getStaticPaths` function is then declared. First we .... then... then...
...
<Alert status="info">
  The `new XataClient` is instantiated within the `getStaticPaths` and not
  within the top-level scope because only imports are available within the scope
  of `getStaticPaths`.
</Alert>

TODO: detail on `getStaticPaths` ^^^^^^^^^
