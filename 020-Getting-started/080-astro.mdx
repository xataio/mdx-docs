...
[TypeScript SDK](https://xata.io/docs/typescript-client/overview). This will additionally be referenced in the `.xatarc`
file as follows:
...
```astro title="src/layouts/main.astro" {2,9,49}
---
...
A single post is accessed via the `slug` value of the post in the application's URL. For example, `/posts/awesome-blog-post`
where `awesome-blog-post` is the slug. This is achieved using Astro's
