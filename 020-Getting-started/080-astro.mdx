...
With the Xata CLI installed and logged in and a new Astro application in place, use the Xata CLI to create a new
...
A single post is accessed via the `slug` value of the post in the application's URL. For example, `/posts/awesome-blog-post` where `awesome-blog-post` is the slug. This is achieved using Astro's
[dynamic routes](https://docs.astro.build/en/core-concepts/routing/#dynamic-routes). You identify parameters within Astro dynamic routes by a name within square brackets within the file path. For example, `lang` and `version` within `src/pages/[lang]/[version]/info.astro`.

Astro is in [SSG (Static Site Generator) mode](https://docs.astro.build/en/core-concepts/routing/#static-ssg-mode) by default, meaning that all routes are identified within `getStaticPaths` function, and site content is generated at build time. Since all the blog posts and their `slug` are defined within a table, you can query the table for all posts and inform the framework about all possible routes and content.

Create a new file, `src/pages/posts/[slug].astro` to enable access to a `slug` parameter:

```tsx astro="src/pages/posts/[slug].astro" {2,4,6-18,20-40}
---
import MainLayout from '../../layouts/main.astro'
...
As discussed, this defines all static pages and their content, and how to render the post details for a single blog post view. Let's break it down.
...
Next, the `XataClient` class definition is imported from the auto-generated `xata.ts` file to be used within `getStaticPaths`:
...
The Component Template uses the `MainLayout` component and passes the blog post title, `post.title`, so the page `<title>` contains the post title. Finally, the values for `post.title`, `post.description`, and `post.pubDate` are then used within the UI.
...
any configuration to enable search, you need to use the
...
With `output` set to `hybrid`, static pages continue to work as they did previously. Now we're ready to add the search functionality.

Adding search is as simple as updating the landing page as follows:

```astro title="src/app/page.tsx" {8, 10-15,19-28}
---
import MainLayout from '../layouts/main.astro';
...
Secondly, the landing page should list all blog posts if `search` is an empty string. However, if
...
auto-generated `Posts` property. The value of `search` is passed as the text value to search for, and a
...
a `GET`request in the format`?q={q-value}`. Since this is precisely the behavior required, and the page is already updated
to get the value of `q` via `Astro.url.searchParams.get('q')`, everything is in place.
...
In this guide, you've learned that Astro with SSG and SSR, and Xata are a powerful combination. You created an
...
with Astro's `getStaticPaths` to identify all possible routes and, thus, static files to be generated.
