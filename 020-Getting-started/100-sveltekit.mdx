...
The `loader` function does the same job as the landing page but is updated to take parameters of type `LoaderArgs`.
The `params` property on `LoaderArgs` gives us the slug value from the URL via `params.slug`.
...
Use the values returned from the `loader` within the `V2_MetaFunction` function to display the Post's title
