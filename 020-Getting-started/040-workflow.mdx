...
Xata provides [an official Vercel integration][2] that will automatically tie Xata preview databases to Vercel preview deployments. Enabling the integration will give Xata the ability to dynamically change environment variables (such a `XATA_BRANCH`) on the fly in your Vercel project as GitHub PRs are made. If you were previously using Vercel and manually setting those variables, you will need to remove them before enabling our integration.

Click **Settings** in the top navigation for any Xata database, then click the Vercel integration from the listed options. This directs you to Vercel's website, where you can select the organization you would like to attach to the Xata database. Your personal account and your team's organization account are listed. Permissions are required to adjust the `.env` variables that instruct Vercel previews to point to the correct Xata database preview.
...
1. Create a new Xata branch with `xata branch create my_feature`, then update your `.env` file to point to the `my_feature` branch.
...
Getting Github, Xata and Vercel or Netlify working together typically is easier when you attach Github first, then Vercel or Netlify. If you were previously using Vercel with Xata without our official integration, you'll want to make sure you clear any Xata related environment variables (like `XATA_BRANCH`) you may have set before performing the integration. Trying to apply the integration (which manages those environment variables automatically) onto a project where they already exist may cause permission errors. This is only required for your deployment, and not for local development.

If you are having trouble getting a specific integration working, try removing the integration on the source provider (Github, Vercel or Netlify) from their respective settings interfaces and trying it over.
