...
<Alert status="warning">
Error: Error serializing `.records[0].xata.createdAt` returned from `getServerSideProps` in "/posts".
Reason: `object` ("[object Date]") cannot be serialized as JSON. Please only return JSON serializable data types.
</Alert>


This error is because Xata stores date fields as an object. To solve this issue, the easiest is to use the `toSerializable` function that is provided by the SDK. This will convert the record into a JSON serializable object.
...
Note that the type returned by `toSerializable()` is `JSONData<PostsRecord>`. This essentially means: has all the columns of the Posts table, but the values are JSON serializable. This means they can be passed to the browser while still keeping the end-to-end type safety of TypeScript.
