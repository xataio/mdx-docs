...
<details>
  <summary><code>xata import csv</code> output</summary>
...
<details>
  <summary><code>xata pull main</code> output</summary>
...
```tsx title="src/app/page.tsx" {1-3,6,21-41}
...
with a `searchParams` property. When a search has been performed, `searchParams` will optionally have a `search` property
...
auto-generated `Posts` property. The value of `searchParams.search` is passed as the text value to search for, and a
...
default behavior of a form is to submit a `GET` request to the current URL. Since this is precisely the behavior
we need, we've updated the page function to accept the object with a `searchParams` property already, everything is now in place.
