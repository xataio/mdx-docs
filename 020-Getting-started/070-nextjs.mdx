...
<Alert status="warning">
  Error: Error serializing `.records[0].xata.createdAt` returned from `getServerSideProps` in "/posts". Reason: `object`
...
Note that the type returned by `toSerializable()` is `JSONData<PostsRecord>`. This helper type converts all properties to JSON serializable. This means they can be passed client-side while still keeping the end-to-end type safety of TypeScript.
