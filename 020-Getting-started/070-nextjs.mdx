...
Then, in the `Home` function, use the `xata` client instance to get all the posts stored in the database. You achieve this via the auto-generated `Posts` property, which exposes a number of helper functions. In this case, we use the [`getAll`](/docs/sdk/get#the-typescript-sdk-functions-for-querying) function to get all the Post records.
...
any configuration to enable search, we just need to use the [TypeScript SDK search feature](/docs/sdk/search).
