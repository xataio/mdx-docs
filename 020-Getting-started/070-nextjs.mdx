...
Note that the type on returned by `toSerializable()` is `JSONData<PostsRecord>`. This essentially means: has all the columns of the Posts table, but the values are JSON serializable. This means they can be passed to the browser while still keeping the type safety of TypeScript.
