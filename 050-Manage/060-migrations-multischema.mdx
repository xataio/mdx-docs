...
Multi Schema migrations enable you to have two active database schemas at the same time. You are able to write data to both your new and old schema simultaneously. It also allows you to seamlessly rollback a migration in the case of an unwanted schema change or backfill.
...
If you find something wrong with the new schema or backfill, you can rollback the mgiration. After a rollback, the schema changes from the migration will not be applied and the new schema will be deleted.
