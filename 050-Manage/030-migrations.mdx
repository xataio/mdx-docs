...
For Postgres enabled branches, the Web UI provides a Migration Editor to create reversible migrations using [pgroll](https://github.com/xataio/pgroll) operations.

Reversible migrations address the pain point of having to keep your application code in sync with your database schema by allowing you two active database schemas at once. With Reversible migrations, running a migration means being able to preview the new version of your schema alongside the old one. Reading and writing data in both schemas continues to work normally. As a result of having two active schema versions, rollbacks become seamless operations which is particularly useful in the case of an unwanted schema change or backfill. More information can be found on our [blog post](https://xata.io/blog/schema-multi-version-launch-week).
...
- We only offer this functionality through the UI or via the REST API endpoints [here](/docs/api-reference/db/db_branch_name/migrations).

# Non Postgres enabled branches
