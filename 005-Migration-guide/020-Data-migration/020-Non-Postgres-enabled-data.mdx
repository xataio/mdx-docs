...
3. Create each of your tables as they were in Xata Lite. You can use the query workspace in the UI to execute the `CREATE TABLE` statements. Below is an example CREATE TABLE statement for a Xata Lite table with Xata columns.

```sql
CREATE TABLE xata.catalogs (
    ...
    url text,
    image_url text,
    CONSTRAINT catalogs_xata_id_length_xata_id CHECK ((length(xata_id) < 256))
);
```
