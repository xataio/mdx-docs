...
Ticket: [xataio/xata-py#101](https://github.com/xataio/xata-py/issues/101)
...
PR: [xataio/xata-py#93](https://github.com/xataio/xata-py/issues/93)
...
```python
list = self.client.databases().getDatabaseList()
```

changed to a more concise call in `1.x`:

```python
list = self.client.databases().list()
```
...
PR: [xataio/xata-py#17](https://github.com/xataio/xata-py/issues/17)
