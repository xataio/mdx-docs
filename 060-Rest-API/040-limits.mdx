...
Xata uses store types with individual concurrency limits. The Consistent transactional store is built into every branch while the Search & Analytics store is optional.
...
| Search & Analytics store (optional) | 15                    | 45                   | `aggregate`, `search`, `ask`, `vectorSearch`                        |
...
- If you are not using any of the `aggregate`, `search`, `ask` and `vectorSearch` endpoints, you can opt-out from the Search store at the branch level in the Web UI, thus avoiding storage costs associated with it.
