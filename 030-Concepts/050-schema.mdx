...
Xata databases are described by a **schema**. A schema describes each table in the database, and the relationships between tables. Xata schemas are expressed as code in JSON and used by our internal systems, as well as the [Command Line Interface](/docs/developer-guide/cli) to work with your database.
...
For a more complete example see the [API Guide](/docs/developer-guide/typescript-client/overview).
...
- From the [CLI](/docs/developer-guide/cli#schema) using commands such as `xata schema edit` or `xata schema upload`.
