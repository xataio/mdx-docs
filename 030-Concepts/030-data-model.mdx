...
produced by any other machine learning model. Once you store the embeddings, you can use vector search for a number of use cases, see the [Similarity/Vector search](/docs/typescript-client/vector-search) section for more information.
...
The `file` type is the representation of a file in the Xata database. For every file, the column stores a JSON objects with a predefined schema. The file object is used to upload a file, retrieve its content, as well as update or query its metadata.
...
When all read only fields are requested in a query, the returned file object will look like the following:
...
The `file[]` (file array) type represents a collection of files stored in the same table cell. Every array item is a JSON object similar to the file object, with the addition of an `id` field, which is used to refer the item in the collection.

The `id` is automatically generated if it is not passed during the time of creation.

Example of adding multiple files in a single column value:
...
The `file[]` column type does not support constraints like `not null` or `unique` or parameters like default value. The default value for a `file[]` column is empty array `{}`.
...
It is possible to use your own ID values, by using the [insert record with ID API](/docs/typescript-client/insert#creating-a-record-with-a-given-id).
...
This column is meant to be use for optimistic concurrency control, see the [dedicated section](/docs/typescript-client/update#optimistic-concurrency-control) for more information.
...
At query time, you can easily include columns from the target table by specifying the `"columns"` field in the request. For an example, see [Selecting columns from the linked tables](/docs/typescript-client/get#selecting-columns-from-the-linked-tables).
