...
Data is organized in "tables" which are grouped into "databases". Tables have a strict [schema](/docs/concepts/schema), which contains a list of columns. Xata supports many column types and the type information is used, among other things, to generate type-safe clients.
...
The keys in the JSON are the column names. The values are the data. The coresponding [schema file](/docs/concepts/schema) for the above record looks like this:
