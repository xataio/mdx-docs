...
The keys in the JSON are the column names. The values are the data. The coresponding [schema file](/docs/concepts/schema) for the above record looks like the following:
...
produced by any other machine learning model. Once you store the embeddings, you can use vector search for a number of use cases, see the [Similarity/Vector search](/docs/developer-guide/typescript-client/vector-search) section for more information.

### object

<Alert>
  The `object` type will be [deprecated](#what-is-the-deprecation-period) from June 14th, 2023 and will no longer be
  available after December 13th, 2023. If you are using the `object` type in your workspace, we strongly encourage you
  to review your implementations and update them accordingly, replacing the `object` type with suitable alternatives as
  recommended in our [migration options](#object-type-migration) documentation.
</Alert>
...
It is possible to use your own ID values, by using the [insert record with ID API](/docs/developer-guide/typescript-client/insert#creating-a-record-with-a-given-id).
...
This column is meant to be use for optimistic concurrency control, see the [dedicated section](/docs/developer-guide/typescript-client/update#optimistic-concurrency-control) for more information.
...
At query time, you can easily include columns from the target table by specifying the `"columns"` field in the request. For an example, see [Selecting columns from the linked tables](/docs/developer-guide/typescript-client/get#selecting-columns-from-the-linked-tables).
...
<Alert>
  Support for backwards link navigation in our SDK and UI is currently in development and will be available to users in
  the near future.
</Alert>
...
<TabbedCode tabs={['TypeScript', 'CURL', 'Python']}>
  ```ts
...
As indicated [our roadmap](https://xata.io/roadmap), we are working on introducing support for a dedicated JSON type. As an interim solution, we suggest storing the data from an object column as serialized JSON in a column of type `text`.
