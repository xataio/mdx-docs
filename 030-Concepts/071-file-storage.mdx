...
Caching is enabled for all signed and public URLs. This is to avoid caching data that is secured by API key permissions. This feature is enabled by default and no configuration is necessary. Public access and signed URL content is cached for 4 hours.

As with all cache systems, the fundamental problem is cache invalidation. Namely getting the cache access performance, while responding to content updates and avoid returning stale entries. Xata's approach is to avoid the need of invalidation by design. The files content is presented as dynamic content similar to the data in the database. Therefore clients should dynamically retrieve the access URLs from the database rather than storing them in static resources. Urls become invalid when the target file updates so access will break if they are stored outside the database.
...
For the very common case of storing images, Xata offers a set of transformations which can be applied on-the-fly when retrieving images through the access URLs (public and signed URLs support transformations). Transformations are specified in the URL and their result is cached by the CDN. In the URL the transformation specification is a comma separated list of individual transformations following the /transform/ segment.
...
The only option is `fast`. This slightly reduces latency on a cache miss by selecting a quickest-to-compress file format, at a cost of increased file size and lower image quality. It will usually override the format option and choose JPEG over WebP or AVIF.
