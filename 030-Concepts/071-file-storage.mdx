...
````ts|python|json
  ```ts
  const photos = await xata.db.Users
    .select(["name", "photo.url", "photo.size"])
...
````ts|python|shell
  ...
  await xata.files.upload(
    { table: "table_name", column: "column_name", record: "record_id" },
    file
  );
  ```
  ```python
...
````ts|python|shell
  ```ts
  await xata.files.upload(
    { table: "table_name", column: "column_name", record: "record_id", fileId: "id" },
    file
  );
  ```
  ```python
...
````ts|python|shell
  ```ts
  const file = await xata.files.download(
    { table: "table_name", column: "column_name", record: "record_id", fileId: "file_id" },
  );
  ```
  ```python
