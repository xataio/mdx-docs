...
The version field for a record is stored in a special column, which is exposed in the Xata API because it is also useful for [optimistic concurrency control](/docs/developer-guide/typescript-client/update#optimistic-concurrency-control).
