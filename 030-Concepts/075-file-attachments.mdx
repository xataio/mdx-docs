...
An upload URL offers authenticated access to upload/update a file without requiring an API key. The URL contains an expiring signature allowing anyone holding the URL to upload a file during the configured duration. Similar to signed URLs, they need to be requested directly and do not come along with the default request on the record.
Http `PUT` is the only valid method for a request using the upload URL.
Note that an upload URL targets a specific file, therefore multiple uploads using the same URL will replace the content of the same file rather than creating multiple files.
...
<TabbedCode tabs={['TypeScript', 'Python']}>
  ...
  const user = await xata.db.Users.update('record_id', {
    photo: {
      uploadUrlTimeout: 600 // In seconds
    }
  });
