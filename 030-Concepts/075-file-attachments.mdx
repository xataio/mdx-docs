...
<Alert status="warning">
  **Public object caching**  
  When a public object gets cached, changing permissions will not invalidate the cached entry. The file is still accessible through old URLs until the cache entry expires in 2h. Xata advises greatest caution when configuring public access, because in practice there is a delay in changing the permissions back from public to private.
</Alert>
