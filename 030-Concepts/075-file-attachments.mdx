...
- [`file`](/docs/concepts/data-model#file) column type
...
- [`file[]`](/docs/concepts/data-model#file) (file array) column type
...
Requests for these URLs require the Xata API key to be present in the Authorization header. Authenticated URLs are retrieved by reading or querying the `url` field of a file object. The `enablePublicUrl` field must be set to `false` (default value) to retrieve an authenticated URL.
