...
import type { LinksFunction } from '@remix-run/node'; // [!code highlight]
import { Links, LiveReload, Meta, Outlet, Scripts, ScrollRestoration } from '@remix-run/react';

import styles from './tailwind.css'; // [!code highlight]

export const links: LinksFunction = () => [{ rel: 'stylesheet', href: styles }]; // [!code highlight]
...
```tsx {2,11,13,15}
export default function Post() {
  const { post } = useLoaderData<typeof loader>(); // [!code highlight]
  ...
  return (
    <div className="w-full max-w-5xl mt-16">
      ...
      <p className="text-sm mb-4 text-purple-950 dark:text-purple-200">
        {new Date(post?.pubDate ?? '').toDateString()} // [!code highlight]
      </p>
...
import { LoaderFunctionArgs, MetaFunction, json } from '@remix-run/node' // [!code highlight]
import { useLoaderData } from '@remix-run/react'
import { getXataClient } from 'src/xata'

...

export async function loader({ request }: LoaderFunctionArgs) { // [!code highlight]
 ...
 // [!code highlight]
  ...
  if (search) { // [!code highlight]
    posts = await xata.db.Posts.search(search, { fuzziness: 2 }) // [!code highlight]
  } else { // [!code highlight]
    posts = await xata.db.Posts.getAll() // [!code highlight]
  } // [!code highlight]
 // [!code highlight]
  return json({ // [!code highlight]
...
export default function Index() {
  const { posts, search } = useLoaderData<typeof loader>() // [!code highlight]
      ...
      <div className="w-full max-w-5xl mt-16"> // [!code highlight]
        <form> // [!code highlight]
          <input // [!code highlight]
            ...
            defaultValue={search || ''} // [!code highlight]
            placeholder="Search..." // [!code highlight]
            className="w-full rounded-lg p-2 border-2 dark:text-purple-950" // [!code highlight]
          /> // [!code highlight]
        </form> // [!code highlight]
      </div> // [!code highlight]
