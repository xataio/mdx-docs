...
module.exports = {
  content: ['./src/**/*.{js,jsx,ts,tsx}'], // [!code highlight]
  theme: {
    extend: {}
  },
  plugins: []
...
import { useLocation } from '@solidjs/router'; // [!code highlight]
import { Match, Switch, For } from 'solid-js';
import { useRouteData } from 'solid-start';
import { createServerData$ } from 'solid-start/server';
...
export function routeData() {
  return createServerData$(async () => {
    ...
    const location = useLocation(); // [!code highlight]
    const search = location.query.q; // [!code highlight]
 // [!code highlight]
    ...
    return { posts, search }; // [!code highlight]
  });
}
...
export default function Home() {
  ...
  const data = resource();
  const posts = data?.posts;
  const search = data?.search || ''; // [!code highlight]
