---
title: Summarize with TypeScript
navTitle: Summarize
keywords: ['TypeScript', 'TypeScript client', 'summarize', 'calculations']
description: Perform calculations and get insights on a set of records using the Xata SDK with TypeScript or JSON
...
<TabbedCode tabs={['TypeScript', 'JSON', 'SQL']}>
  ```ts
...
```jsonc
// POST https://tutorial-ng7s8c.us-east-1.xata.sh/db/tutorial:main/sql

{
    "query": "SELECT product,AVG(sale_price) FROM \"sales\" WHERE product = 'basketball' GROUP BY product;"
}
```
...
{
    "query":  "SELECT product,SUM(sale_price) FROM \"sales\" GROUP BY product;"
}
```
...
{
    "query": "SELECT username,COUNT(username) FROM \"sales\" GROUP BY username;"
}
```
...
Filter lets you filter data before grouping and summarizing. Check out our guide [here](/docs/typescript-client/filtering).
...
{
    "query": "SELECT store_name,COUNT(*) AS total_sales FROM \"sales\" GROUP BY store_name ORDER BY total_sales desc, store_name asc;"
}
```
...
If this limitation is too small, we ask that you move your workload to the [aggregate](/docs/typescript-client/aggregate) endpoint which does not have such limitations.
...
{
    "query": "SELECT product_name, SUM(price) AS revenue, COUNT(*) AS items_sold FROM \"titles\" GROUP BY product_name HAVING SUM(price) >= 100 AND COUNT(*) >= 1000;"
}
```
...
{
    "query": "SELECT product, AVG(sale_price) AS average_sale_price FROM \"sales\" WHERE product = 'basketball' GROUP BY product;",
    "consistency": "eventual"
}
```
...
The [aggregate](/docs/typescript-client/aggregate) endpoint will almost always perform better. It's quicker to handle larger tables, with more cardinality, and does not have a hard limit on the amount of results that one can request in a single request. See the below section "Summarize or Aggregate" to understand why.

## Summarize or Aggregate?

Summarize and [aggregate](/docs/typescript-client/aggregate) both return similar results. They differ in terms of the underlying store the data is served from. When using summarize, one retrieves data from PostgreSQL. This means that you will retrieve consistent results.
...
Note: there are some minor differences to how the two endpoints function. Please see the [documentation for aggregate](/docs/typescript-client/aggregate) for more information.
