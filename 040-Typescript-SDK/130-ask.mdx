---
title: Asking questions of your data with AI using TypeScript
navTitle: Ask AI
keywords: ['TypeScript', 'TypeScript client', 'AI', 'artificial intelligence', 'language model']
description: Use TypeScript and AI to ask meaningful questions and extract valuable insights from your data
slug: typescript-client/ask
published: true
---

The `ask` endpoint helps you create interactive and engaging conversational interfaces, Q&A assistants, and chatbots. It gives users the information they need quickly and directly, letting them engage with your product without having to wade through documentation on articles. It uses up-to-date information from your database inside of Xata. This could be your documentation, team knowledge-base, or any source of data you've set up. The ask endpoint uses Xata's search capabilities to find relevant information from your database, and uses this with OpenAI's ChatGPT API to interpret your question and to generate natural-sounding responses.
...
- Take the most relevant results, and create a custom prompt for OpenAI.
...
{
  "answer": /*<< answer >>*/,
  "session_id": "cg52bk1eqh5rd5hndhq95jercs",
  "records": [
...
The options that you can pass under the `search` key mirror a subset of the options from the [search endpoint](/docs/typescript-client/search#searching-in-a-single-table). See that section of the guide for more details.
...
The options that you can pass under the `search` key mirror a subset of the options from the [vector search endpoint](/docs/typescript-client/vector-search). See that section of the guide for more details.
...
Xata supports asking follow-up questions, enabling ChatGPT-style interfaces for your data. Here, we have the concept of a **session**. A session represents a single conversation between a user and data in one of your Xata tables. A session has data from your tables, aka **context**. It also persists the rules you defined when starting the session, and all question sent and answers received.

When communicating with OpenAI, the ask endpoint ensures that your rules, your context, and as many questions as possible are transmitted each time. If the total amount of messages becomes too long for OpenAI to process, Xata will remove messages from early in the conversation until the message size is small enough to work.
...
```ts
const result = await xata.db.Tutorial.askTableContinue('<session_id>', { message: '<your follow-up message>' });
```

```jsonc
// POST https://tutorial-ng7s8c.us-east-1.xata.sh/db/tutorial:main/tables/Tutorial/ask/<session_id>
{
...
```jsonc
// POST https://tutorial-ng7s8c.us-east-1.xata.sh/db/tutorial:main/tables/Tutorial/ask/cg52bk1eqh5rd5hndhq95jercs
{
...
From which you will receive a response with the same structure as below.
...
Please check the [pricing](<[pricing](https://xata.io/pricing)>) page for the details.

### What data is given to OpenAI?

If you never use the ask endpoint, your data will **never** be sent to OpenAI.

When you use the endpoint, Xata finds the most relevant fields matching your search query and sends them in fragments or in their entirety to OpenAI. This is done per-request. No data is synced on any kind of schedule, and only data in the table you search for and matching the parameters you provide can be sent.
