...
The SDK provides helper methods for creating types based off of the Xata client-generated types.
These are exported on the `@xata.io/client` package: `SelectedPick` , `SelectableColumn` and `EditableData`.
...
The following example creates the type `myUser` with a selection of fields from the client-generated UserRecord:
