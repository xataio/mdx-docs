...
navTitle: GitHub branching workflow
...
<Alert status="warning">
  **This feature is in a beta state**. We are currently rebuilding our migration strategy to incorporate
  [pgroll](https://github.com/xataio/pgroll) alongside native Postgres support. Until then, migrations and branching as
  documented below are limited in scope. Specifically, the GitHub integration does not work within a monorepo with
  multiple Xata databases, and File attachments within records will not get copied to generated preview branches.
</Alert>
